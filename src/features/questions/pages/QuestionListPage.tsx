...
import { Difficulty, QuestionStatus, Topic } from '../../../constants/enums';
...
export const QuestionListPage = () => {
  ...
  // Focus search input if focusSearch param is present
  useEffect(() => {
  ...
  const allQuestions = Object.values(questionsData).map((q) => ({
    ...
    topic: q.topic,
    status: q.id === 1 ? QuestionStatus.SOLVED : QuestionStatus.NOT_STARTED, // Two Sum is marked as solved
    tags: q.tags || [],
  }));
  ...
  const questions = allQuestions.filter((q) => {
    ...
    // Status filter
    if (status !== 'all') {
      if (status === 'viewed' && q.status === QuestionStatus.NOT_STARTED) {
        return false;
      }
      if (status === 'notViewed' && q.status !== QuestionStatus.NOT_STARTED) {
        return false;
      }
    }
  ...
  const STATUS_COLORS = {
    [QuestionStatus.SOLVED]: '#10b981',
    [QuestionStatus.ATTEMPTED]: '#f59e0b',
    [QuestionStatus.NOT_STARTED]: themeColors.gray400,
  };
    ...
    switch (status) {
      case QuestionStatus.SOLVED:
        return (
          ...
          >
            <CheckCircle sx={{ color: STATUS_COLORS[QuestionStatus.SOLVED], fontSize: 20 }} />
          </Box>
        );
  ...
  const DIFFICULTY_COLORS = {
    [Difficulty.EASY]: { bg: '#10b98133', text: '#10b981' },
    [Difficulty.MEDIUM]: { bg: '#f59e0b33', text: '#f59e0b' },
    [Difficulty.HARD]: { bg: '#ef444433', text: '#ef4444' },
  };
              ...
              <TableBody>
                {questions.map((question, index) => {
                  ...
                  return (
                    <TableRow
                      key={question.id}
                      onClick={() => navigate(ROUTES.PROBLEM_DESCRIPTION(question.slug))}
                      sx={{
                        height: '64px',
                        cursor: 'pointer',
                        '&:hover': {
